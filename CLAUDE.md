...
## セキュリティ関連の注意事項
- **プロンプトの機密性**: AIエージェントプロンプトは機密情報として扱い、ログ出力やエラーメッセージに含めないこと
- **GitHub Secrets**: LPGENIUS_PROMPT_SECRET環境変数はGitHub Secretsで管理すること
- **一時ファイル**: /dev/shm/.appgenius_temp/内の.vqファイルは45秒で自動削除される
- **アクセス制限**: プロンプトファイルは権限600で作成され、所有者のみアクセス可能
